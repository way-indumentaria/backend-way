...
function obtenerVentasImpagasPagas(fdesde, fhasta) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const ventas_impagas = yield db.query("SELECT vd.producto,sum(p.precio_way*vd.cantidad)as importe ,vip.vendedor, vd.fecha_venta FROM producto p, venta_detalle vd, venta_impaga_paga vip where vd.producto = p.id_producto and vd.id_venta_paga_impaga = vip.id_impaga_paga and vd.fecha_venta >= ? and vd.fecha_venta <= ? and vd.estado = 0 GROUP by MONTH(vd.fecha_venta),YEAR(vd.fecha_venta)", [fdesde, fhasta]);
            const ventas_pagas = yield db.query("SELECT vd.producto,sum(p.precio_way*vd.cantidad)as importe ,vip.vendedor, vd.fecha_venta FROM producto p, venta_detalle vd, venta_impaga_paga vip where vd.producto = p.id_producto and vd.id_venta_paga_impaga = vip.id_impaga_paga and vd.fecha_venta >= ? and vd.fecha_venta <= ? and vd.estado = 1 GROUP by MONTH(vd.fecha_venta),YEAR(vd.fecha_venta)", [fdesde, fhasta]);
...
function obtenerVentasTotales(fdesde, fhasta) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const db = yield database_1.conexion();
            //const ventas:any[] = await db.query('select ven.fecha_venta , sum((p.precio_way*ven.cantidad)-(p.precio_compra*ven.cantidad)) as importe from venta_detalle ven, producto p where p.id_producto = ven.producto and ven.fecha_venta BETWEEN ? AND ? and ven.estado = 1 group by MONTH(ven.fecha_venta)',[fdesde,fhasta]);
            const ventas = yield db.query("SELECT vd.producto,sum((p.precio_way*vd.cantidad)-(p.precio_compra*vd.cantidad))as importe ,vip.vendedor, vd.fecha_venta FROM producto p, venta_detalle vd, venta_impaga_paga vip where vd.producto = p.id_producto and vd.id_venta_paga_impaga = vip.id_impaga_paga and vd.fecha_venta >= ? and vd.fecha_venta <= ? and vd.estado = 1 GROUP by MONTH(vd.fecha_venta),YEAR(vd.fecha_venta)", [fdesde, fhasta]);
            if (ventas) {
                let ventas_array = [];
...
function obtenerVentasVendedores(fdesde, fhasta, vendedor) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const resultado = yield db.query('SELECT vd.producto,sum(p.precio_way*vd.cantidad)as importe ,vip.vendedor, vd.fecha_venta FROM producto p, venta_detalle vd, venta_impaga_paga vip where vd.producto = p.id_producto and vd.id_venta_paga_impaga = vip.id_impaga_paga and vip.vendedor = ? and vd.fecha_venta >= ? and vd.fecha_venta <= ? and vd.estado = 1 GROUP by vip.vendedor,MONTH(vd.fecha_venta),YEAR(vd.fecha_venta)', [vendedor, fdesde, fhasta]);
...
function obtenerImapagasPagasGastosTotales(fdesde, fhasta) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const db = yield database_1.conexion();
            //const ventas:any[] = await db.query('select ven.fecha_venta , sum((p.precio_way*ven.cantidad)-(p.precio_compra*ven.cantidad)) as importe from venta_detalle ven, producto p where p.id_producto = ven.producto and ven.fecha_venta BETWEEN ? AND ? and ven.estado = 1 group by MONTH(ven.fecha_venta)',[fdesde,fhasta]);
            const ventas_impagas = yield db.query("SELECT sum(p.precio_way*vd.cantidad)as importe FROM producto p, venta_detalle vd, venta_impaga_paga vip where vd.producto = p.id_producto and vd.id_venta_paga_impaga = vip.id_impaga_paga and vd.fecha_venta >= ? and vd.fecha_venta <= ? and vd.estado = 0", [fdesde, fhasta]);
            const ventas_pagas = yield db.query("SELECT sum(p.precio_way*vd.cantidad)as importe FROM producto p, venta_detalle vd, venta_impaga_paga vip where vd.producto = p.id_producto and vd.id_venta_paga_impaga = vip.id_impaga_paga and vd.fecha_venta >= ? and vd.fecha_venta <= ? and vd.estado = 1", [fdesde, fhasta]);
